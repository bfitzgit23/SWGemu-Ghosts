/*
 *	autogen/server/zone/objects/area/FsVillageArea.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_NOTIFYENTER__SCENEOBJECT_ = 312141493};
...
DistributedObjectServant* FsVillageArea::_getImplementationForRead() const {
	return _impl;
}
...
void FsVillageAreaImplementation::writeJSON(nlohmann::json& j) {
...
DistributedObjectPOD* FsVillageAreaHelper::instantiatePOD() {
	return new FsVillageAreaPOD();
}
...
FsVillageAreaPOD::~FsVillageAreaPOD() {
	finalize();
}

FsVillageAreaPOD::FsVillageAreaPOD(void) {
	_className = "FsVillageArea";
}


void FsVillageAreaPOD::writeJSON(nlohmann::json& j) {
...
void FsVillageAreaPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = FsVillageAreaPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int FsVillageAreaPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool FsVillageAreaPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void FsVillageAreaPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(FsVillageAreaPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void FsVillageAreaPOD::writeObjectCompact(ObjectOutputStream* stream) {
