/*
 *	autogen/server/zone/objects/area/BadgeActiveArea.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_NOTIFYENTER__SCENEOBJECT_ = 790259300,RPC_SETBADGE__INT_,RPC_GETBADGE__};
...
DistributedObjectServant* BadgeActiveArea::_getImplementationForRead() const {
	return _impl;
}
...
void BadgeActiveAreaImplementation::writeJSON(nlohmann::json& j) {
	ActiveAreaImplementation::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	thisObject["areaBadge"] = areaBadge;

	j["BadgeActiveArea"] = thisObject;
}
...
void BadgeActiveAreaImplementation::notifyEnter(SceneObject* player) {
	...
	ManagedReference<CreatureObject* > playerCreature = dynamic_cast<CreatureObject*>(player);
...
unsigned int BadgeActiveAreaImplementation::getBadge() const{
...
void BadgeActiveAreaAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_SETBADGE__INT_:
		{
			unsigned int a = inv->getUnsignedIntParameter();
...
unsigned int BadgeActiveAreaAdapter::getBadge() const {
...
DistributedObjectPOD* BadgeActiveAreaHelper::instantiatePOD() {
	return new BadgeActiveAreaPOD();
}
...
BadgeActiveAreaPOD::~BadgeActiveAreaPOD() {
	finalize();
}

BadgeActiveAreaPOD::BadgeActiveAreaPOD(void) {
	_className = "BadgeActiveArea";
}


void BadgeActiveAreaPOD::writeJSON(nlohmann::json& j) {
	ActiveAreaPOD::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	if (areaBadge)
		thisObject["areaBadge"] = areaBadge.value();

	j["BadgeActiveArea"] = thisObject;
}


void BadgeActiveAreaPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = BadgeActiveAreaPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int BadgeActiveAreaPOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<unsigned int >::toBinaryStream(&areaBadge.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool BadgeActiveAreaPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0xc4a86e4a: //BadgeActiveArea.areaBadge
		{
			unsigned int _mnareaBadge;
			TypeInfo<unsigned int >::parseFromBinaryStream(&_mnareaBadge, stream);
			areaBadge = std::move(_mnareaBadge);
		}
		return true;
...
void BadgeActiveAreaPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(BadgeActiveAreaPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void BadgeActiveAreaPOD::writeObjectCompact(ObjectOutputStream* stream) {
