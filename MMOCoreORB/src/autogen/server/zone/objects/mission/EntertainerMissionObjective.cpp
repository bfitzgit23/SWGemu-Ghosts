/*
 *	autogen/server/zone/objects/mission/EntertainerMissionObjective.cpp generated by engine3 IDL compiler 0.70
 */
...
DistributedObjectServant* EntertainerMissionObjective::_getImplementationForRead() const {
	return _impl;
}
...
DistributedObjectPOD* EntertainerMissionObjectiveHelper::instantiatePOD() {
	return new EntertainerMissionObjectivePOD();
}
...
EntertainerMissionObjectivePOD::~EntertainerMissionObjectivePOD() {
	finalize();
}

EntertainerMissionObjectivePOD::EntertainerMissionObjectivePOD(void) {
	_className = "EntertainerMissionObjective";
}


void EntertainerMissionObjectivePOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = EntertainerMissionObjectivePOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int EntertainerMissionObjectivePOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<bool >::toBinaryStream(&inMissionArea.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<bool >::toBinaryStream(&isEntertaining.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}

	if (locationActiveArea) {
	_nameHashCode = 0xfe92104c; //EntertainerMissionObjective.locationActiveArea
	TypeInfo<uint32>::toBinaryStream(&_nameHashCode, stream);
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<ManagedReference<ActiveAreaPOD* > >::toBinaryStream(&locationActiveArea.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<Reference<CompleteMissionAfterCertainTimeTask* > >::toBinaryStream(&completeTask.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool EntertainerMissionObjectivePOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0xc21898c6: //EntertainerMissionObjective.inMissionArea
		{
			bool _mninMissionArea;
			TypeInfo<bool >::parseFromBinaryStream(&_mninMissionArea, stream);
			inMissionArea = std::move(_mninMissionArea);
		}
		return true;

	case 0x4160eb5d: //EntertainerMissionObjective.isEntertaining
		{
			bool _mnisEntertaining;
			TypeInfo<bool >::parseFromBinaryStream(&_mnisEntertaining, stream);
			isEntertaining = std::move(_mnisEntertaining);
		}
		return true;

	case 0xfe92104c: //EntertainerMissionObjective.locationActiveArea
		{
			ManagedReference<ActiveAreaPOD* > _mnlocationActiveArea;
			TypeInfo<ManagedReference<ActiveAreaPOD* > >::parseFromBinaryStream(&_mnlocationActiveArea, stream);
			locationActiveArea = std::move(_mnlocationActiveArea);
		}
		return true;

	case 0xb6e6d429: //EntertainerMissionObjective.completeTask
		{
			Reference<CompleteMissionAfterCertainTimeTask* > _mncompleteTask;
			TypeInfo<Reference<CompleteMissionAfterCertainTimeTask* > >::parseFromBinaryStream(&_mncompleteTask, stream);
			completeTask = std::move(_mncompleteTask);
		}
		return true;
...
void EntertainerMissionObjectivePOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(EntertainerMissionObjectivePOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void EntertainerMissionObjectivePOD::writeObjectCompact(ObjectOutputStream* stream) {
