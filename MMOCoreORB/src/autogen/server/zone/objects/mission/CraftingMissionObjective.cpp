/*
 *	autogen/server/zone/objects/mission/CraftingMissionObjective.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_FINALIZE__ = 3358219931,RPC_INITIALIZETRANSIENTMEMBERS__,RPC_UPDATEMISSIONSTATUS__CREATUREOBJECT_,RPC_ABORT__};
...
DistributedObjectServant* CraftingMissionObjective::_getImplementationForRead() const {
	return _impl;
}
...
void CraftingMissionObjectiveAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_ABORT__:
		...
		break;
	default:
		DeliverMissionObjectiveAdapter::invokeMethod(methid, inv);
	}
}
...
DistributedObjectPOD* CraftingMissionObjectiveHelper::instantiatePOD() {
	return new CraftingMissionObjectivePOD();
}
...
CraftingMissionObjectivePOD::~CraftingMissionObjectivePOD() {
	finalize();
}

CraftingMissionObjectivePOD::CraftingMissionObjectivePOD(void) {
	_className = "CraftingMissionObjective";
}


void CraftingMissionObjectivePOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = CraftingMissionObjectivePOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int CraftingMissionObjectivePOD::writeObjectMembers(ObjectOutputStream* stream) {
	int _count = DeliverMissionObjectivePOD::writeObjectMembers(stream);
...
bool CraftingMissionObjectivePOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	if (DeliverMissionObjectivePOD::readObjectMember(stream, nameHashCode))
		return true;
...
void CraftingMissionObjectivePOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(CraftingMissionObjectivePOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void CraftingMissionObjectivePOD::writeObjectCompact(ObjectOutputStream* stream) {
	DeliverMissionObjectivePOD::writeObjectCompact(stream);
