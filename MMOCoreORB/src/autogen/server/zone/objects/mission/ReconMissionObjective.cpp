/*
 *	autogen/server/zone/objects/mission/ReconMissionObjective.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_FINALIZE__ = 1869515498,RPC_INITIALIZETRANSIENTMEMBERS__,RPC_ACTIVATE__,RPC_ABORT__,RPC_COMPLETE__};
...
DistributedObjectServant* ReconMissionObjective::_getImplementationForRead() const {
	return _impl;
}
...
void ReconMissionObjectiveImplementation::writeJSON(nlohmann::json& j) {
	MissionObjectiveImplementation::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	thisObject["locationActiveArea"] = locationActiveArea;

	j["ReconMissionObjective"] = thisObject;
}
...
DistributedObjectPOD* ReconMissionObjectiveHelper::instantiatePOD() {
	return new ReconMissionObjectivePOD();
}
...
ReconMissionObjectivePOD::~ReconMissionObjectivePOD() {
	finalize();
}

ReconMissionObjectivePOD::ReconMissionObjectivePOD(void) {
	_className = "ReconMissionObjective";
}


void ReconMissionObjectivePOD::writeJSON(nlohmann::json& j) {
	MissionObjectivePOD::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	if (locationActiveArea)
		thisObject["locationActiveArea"] = locationActiveArea.value();

	j["ReconMissionObjective"] = thisObject;
}


void ReconMissionObjectivePOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = ReconMissionObjectivePOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int ReconMissionObjectivePOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<ManagedReference<MissionReconActiveAreaPOD* > >::toBinaryStream(&locationActiveArea.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool ReconMissionObjectivePOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0x938a0110: //ReconMissionObjective.locationActiveArea
		{
			ManagedReference<MissionReconActiveAreaPOD* > _mnlocationActiveArea;
			TypeInfo<ManagedReference<MissionReconActiveAreaPOD* > >::parseFromBinaryStream(&_mnlocationActiveArea, stream);
...
void ReconMissionObjectivePOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(ReconMissionObjectivePOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void ReconMissionObjectivePOD::writeObjectCompact(ObjectOutputStream* stream) {
