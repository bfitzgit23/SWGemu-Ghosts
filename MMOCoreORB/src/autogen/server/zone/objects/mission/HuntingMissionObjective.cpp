/*
 *	autogen/server/zone/objects/mission/HuntingMissionObjective.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_FINALIZE__ = 2185561123,RPC_INITIALIZETRANSIENTMEMBERS__,RPC_ACTIVATE__,RPC_ABORT__,RPC_COMPLETE__,RPC_NOTIFYOBSERVEREVENT__MISSIONOBSERVER_INT_OBSERVABLE_MANAGEDOBJECT_LONG_};
...
DistributedObjectServant* HuntingMissionObjective::_getImplementationForRead() const {
	return _impl;
}
...
void HuntingMissionObjectiveImplementation::writeJSON(nlohmann::json& j) {
	MissionObjectiveImplementation::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	thisObject["targetsKilled"] = targetsKilled;

	j["HuntingMissionObjective"] = thisObject;
}
...
DistributedObjectPOD* HuntingMissionObjectiveHelper::instantiatePOD() {
	return new HuntingMissionObjectivePOD();
}
...
HuntingMissionObjectivePOD::~HuntingMissionObjectivePOD() {
	finalize();
}

HuntingMissionObjectivePOD::HuntingMissionObjectivePOD(void) {
	_className = "HuntingMissionObjective";
}


void HuntingMissionObjectivePOD::writeJSON(nlohmann::json& j) {
	MissionObjectivePOD::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	if (targetsKilled)
		thisObject["targetsKilled"] = targetsKilled.value();

	j["HuntingMissionObjective"] = thisObject;
}


void HuntingMissionObjectivePOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = HuntingMissionObjectivePOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int HuntingMissionObjectivePOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<int >::toBinaryStream(&targetsKilled.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool HuntingMissionObjectivePOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0x557682a3: //HuntingMissionObjective.targetsKilled
		{
			int _mntargetsKilled;
			TypeInfo<int >::parseFromBinaryStream(&_mntargetsKilled, stream);
			targetsKilled = std::move(_mntargetsKilled);
		}
		return true;
...
void HuntingMissionObjectivePOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(HuntingMissionObjectivePOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void HuntingMissionObjectivePOD::writeObjectCompact(ObjectOutputStream* stream) {
