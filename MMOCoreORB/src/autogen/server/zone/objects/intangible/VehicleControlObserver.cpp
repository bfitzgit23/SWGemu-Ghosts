/*
 *	autogen/server/zone/objects/intangible/VehicleControlObserver.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_NOTIFYOBSERVEREVENT__INT_OBSERVABLE_MANAGEDOBJECT_LONG_ = 1412597874};
...
DistributedObjectServant* VehicleControlObserver::_getImplementationForRead() const {
	return _impl;
}
...
int VehicleControlObserverImplementation::notifyObserverEvent(unsigned int eventType, Observable* observable, ManagedObject* arg1, long long arg2) {
	...
	// server/zone/objects/intangible/VehicleControlObserver.idl():  			return 0;
	return 0;
}
...
DistributedObjectPOD* VehicleControlObserverHelper::instantiatePOD() {
	return new VehicleControlObserverPOD();
}
...
VehicleControlObserverPOD::~VehicleControlObserverPOD() {
}

VehicleControlObserverPOD::VehicleControlObserverPOD(void) {
	_className = "VehicleControlObserver";
}


void VehicleControlObserverPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = VehicleControlObserverPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int VehicleControlObserverPOD::writeObjectMembers(ObjectOutputStream* stream) {
	int _count = ObserverPOD::writeObjectMembers(stream);

	uint32 _nameHashCode;
	int _offset;
	uint32 _totalSize;
	if (vehicleControlDevice) {
	_nameHashCode = 0x215ae115; //VehicleControlObserver.vehicleControlDevice
	TypeInfo<uint32>::toBinaryStream(&_nameHashCode, stream);
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<ManagedWeakReference<VehicleControlDevicePOD* > >::toBinaryStream(&vehicleControlDevice.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool VehicleControlObserverPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0x215ae115: //VehicleControlObserver.vehicleControlDevice
		{
			ManagedWeakReference<VehicleControlDevicePOD* > _mnvehicleControlDevice;
			TypeInfo<ManagedWeakReference<VehicleControlDevicePOD* > >::parseFromBinaryStream(&_mnvehicleControlDevice, stream);
			vehicleControlDevice = std::move(_mnvehicleControlDevice);
		}
		return true;
...
void VehicleControlObserverPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(VehicleControlObserverPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void VehicleControlObserverPOD::writeObjectCompact(ObjectOutputStream* stream) {
