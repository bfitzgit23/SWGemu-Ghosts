/*
 *	autogen/server/zone/objects/ship/FighterShipObject.cpp generated by engine3 IDL compiler 0.70
 */
...
DistributedObjectServant* FighterShipObject::_getImplementationForRead() const {
	return _impl;
}
...
void FighterShipObjectImplementation::readObject(ObjectInputStream* stream) {
	...
	for (int i = 0; i < _varCount; ++i) {
		...
		if(FighterShipObjectImplementation::readObjectMember(stream, _nameHashCode)) {
		}
...
bool FighterShipObjectImplementation::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
DistributedObjectPOD* FighterShipObjectHelper::instantiatePOD() {
	return new FighterShipObjectPOD();
}
...
FighterShipObjectPOD::~FighterShipObjectPOD() {
	finalize();
}
...
void FighterShipObjectPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = FighterShipObjectPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int FighterShipObjectPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool FighterShipObjectPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void FighterShipObjectPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(FighterShipObjectPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void FighterShipObjectPOD::writeObjectCompact(ObjectOutputStream* stream) {
