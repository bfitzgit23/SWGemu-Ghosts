/*
 *	autogen/server/zone/objects/installation/components/TurretObserver.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_NOTIFYOBSERVEREVENT__INT_OBSERVABLE_MANAGEDOBJECT_LONG_ = 875304273};
...
DistributedObjectServant* TurretObserver::_getImplementationForRead() const {
	return _impl;
}
...
void TurretObserverImplementation::writeJSON(nlohmann::json& j) {
...
DistributedObjectPOD* TurretObserverHelper::instantiatePOD() {
	return new TurretObserverPOD();
}
...
TurretObserverPOD::~TurretObserverPOD() {
}

TurretObserverPOD::TurretObserverPOD(void) {
	_className = "TurretObserver";
}


void TurretObserverPOD::writeJSON(nlohmann::json& j) {
...
void TurretObserverPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = TurretObserverPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int TurretObserverPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool TurretObserverPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void TurretObserverPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(TurretObserverPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void TurretObserverPOD::writeObjectCompact(ObjectOutputStream* stream) {
