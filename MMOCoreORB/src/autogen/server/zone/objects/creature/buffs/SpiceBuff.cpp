/*
 *	autogen/server/zone/objects/creature/buffs/SpiceBuff.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_ACTIVATE__BOOL_ = 3268252920,RPC_DEACTIVATE__BOOL_,RPC_SETDOWNERATTRIBUTES__CREATUREOBJECT_BUFF_};
...
DistributedObjectServant* SpiceBuff::_getImplementationForRead() const {
	return _impl;
}
...
void SpiceBuffImplementation::writeJSON(nlohmann::json& j) {
	...
	case RPC_SETDOWNERATTRIBUTES__CREATUREOBJECT_BUFF_:
		{
			...
			setDownerAttributes(creature, buff);
...
DistributedObjectPOD* SpiceBuffHelper::instantiatePOD() {
	return new SpiceBuffPOD();
}
...
SpiceBuffPOD::~SpiceBuffPOD() {
}

SpiceBuffPOD::SpiceBuffPOD(void) {
	_className = "SpiceBuff";
}


void SpiceBuffPOD::writeJSON(nlohmann::json& j) {
	BuffPOD::writeJSON(j);
...
void SpiceBuffPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = SpiceBuffPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int SpiceBuffPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool SpiceBuffPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void SpiceBuffPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(SpiceBuffPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void SpiceBuffPOD::writeObjectCompact(ObjectOutputStream* stream) {
