/*
 *	autogen/server/zone/objects/creature/buffs/SpiceDownerBuff.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_ACTIVATE__BOOL_ = 359527136,RPC_DEACTIVATE__BOOL_};
...
DistributedObjectServant* SpiceDownerBuff::_getImplementationForRead() const {
	return _impl;
}
...
void SpiceDownerBuffImplementation::writeJSON(nlohmann::json& j) {
...
void SpiceDownerBuffImplementation::activate(bool applyModifiers) {
	...
	BuffImplementation::creature.getForUpdate().get()->sendSystemMessage("@spice/spice:" + BuffImplementation::buffName + "_downer");
	// server/zone/objects/creature/buffs/SpiceDownerBuff.idl():  		super.
	if (BuffImplementation::creature.getForUpdate().get()->hasBuff(BuffCRC::FOOD_REDUCE_SPICE_DOWNTIME)){
	...
	float reduction = 1.f - (reductionMod / 100);
	// server/zone/objects/creature/buffs/SpiceDownerBuff.idl():  			}
	if (reduction > 0)	// server/zone/objects/creature/buffs/SpiceDownerBuff.idl():  					super.buffDuration = super.buffDuration * reduction;
	BuffImplementation::buffDuration = BuffImplementation::buffDuration * reduction;
}
}
	...
	BuffImplementation::creature.getForUpdate().get()->notifyObservers(ObserverEventType::SPICEDOWNERACTIVATED, BuffImplementation::creature.getForUpdate(), 0);
}
...
DistributedObjectPOD* SpiceDownerBuffHelper::instantiatePOD() {
	return new SpiceDownerBuffPOD();
}
...
SpiceDownerBuffPOD::~SpiceDownerBuffPOD() {
}

SpiceDownerBuffPOD::SpiceDownerBuffPOD(void) {
	_className = "SpiceDownerBuff";
}


void SpiceDownerBuffPOD::writeJSON(nlohmann::json& j) {
	BuffPOD::writeJSON(j);
...
void SpiceDownerBuffPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = SpiceDownerBuffPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int SpiceDownerBuffPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool SpiceDownerBuffPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void SpiceDownerBuffPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(SpiceDownerBuffPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void SpiceDownerBuffPOD::writeObjectCompact(ObjectOutputStream* stream) {
