/*
 *	autogen/server/zone/objects/scene/SceneObject.h generated by engine3 IDL compiler 0.70
 */
...
public:
	...
	void initializeContainerObjectsMap();
	...
	*/
	...
	void createObjectMenuComponent();
	 ...
	 */
	void info(const String& msg, bool forced = false) const;
	 ...
	 * @return returns true if this object is in range with obj
	 */
	bool isInRangeZoneless(SceneObject* obj, float range);
	 ...
	 */
	void removeObjectFromZone(Zone* zone, SceneObject* par);
	...
	Reference<SceneObject* > getCraftedComponentsSatchel();
	 ...
	 * @param name Name of the task
	 */
	void executeOrderedTask(const StdFunction& function, const String& name);
	...
	void showFlyText(const String& file, const String& aux, byte red, byte green, byte blue, bool isPrivate = false);
	...
	bool containsActiveSession(unsigned const int type) const;
	...
	float getDirectionW() const;

	const Quaternion* getDirection() const;

	unsigned int getClientObjectCRC() const;

	unsigned int getServerObjectCRC() const;

	bool isWaypointObject() const;

	const StringId* getObjectName() const;

	String getObjectNameStringIdFile() const;
	...
	const Vector<String>* getArrangementDescriptor(int idx) const;

	String getSlotDescriptor(int idx) const;

	bool hasSlotDescriptor(const String& descr) const;
	...
	int getSlottedObjectsSize() const;

	bool isContainerFull();

	bool isContainerFullRecursive();

	unsigned int getContainerVolumeLimit() const;

	Reference<SceneObject* > getContainerObject(int idx);

	const VectorMap<unsigned long long, ManagedReference<SceneObject* > >* getContainerObjects() const;

	VectorMap<unsigned long long, ManagedReference<SceneObject* > >* getContainerObjects();

	bool hasObjectInContainer(unsigned long long objectID) const;
	...
	long long getLastContainerAccess() const;

	void unloadContainerObjects();
	...
	unsigned int getContainerType() const;
	...
	float getSpecialDirectionAngle() const;
	...
	SceneObject* getRootParentUnsafe();

	void updateSavedRootParentRecursive(SceneObject* newRoot, int maxDepth = 100);
	 ...
	 */
	Reference<SceneObject* > getParentRecursively(unsigned int gameObjectType);
	...
	UnicodeString getCustomObjectName() const;

	String getLoggingName() const;
	...
	bool isTurretControlTerminal() const;
	...
	bool isTrapObject() const;

	bool isRepairTool() const;
	...
	SharedObjectTemplate* getObjectTemplate() const;

	/**
	 * The returning value points directly to the internal vector, do not delete it or store it
	 */
	SortedVector<ManagedReference<Observer* > > getObservers(unsigned int eventType) const;

	void createChildObjects();

	void destroyChildObjects();

	void addChildObject(SceneObject* obj);
...
protected:
	...
	ManagedReference<SceneObject* > savedRootParent;
...
public:
	...
	*/
	...
	virtual void createObjectMenuComponent();
	 ...
	 * @return returns true if the object has been successfully added
	 */
	virtual bool transferObject(SceneObject* object, int containmentType, bool notifyClient = false, bool allowOverflow = false, bool notifyRoot = true);
	...
	virtual Reference<SceneObject* > getCraftedComponentsSatchel();
...
public:
	 ...
	 * @param player SceneObject that will receive the objects
	 */
	virtual void sendContainerObjectsTo(SceneObject* player, bool forceLoad);
	...
	virtual ManagedWeakReference<CityRegion* > getCityRegion() const;
	...
	virtual Reference<ZoneClientSession* > getClient();
	...
	virtual bool isDroidDeedObject();
	...
	virtual bool isCampStructure() const;
	...
	virtual unsigned int getPlanetCRC() const;

	bool isClientObject() const;

	virtual bool isStaticObjectClass();
...
public:
	...
	void showFlyText(const String& file, const String& aux, byte red, byte green, byte blue, bool isPrivate);
	...
	void updateSavedRootParentRecursive(SceneObject* newRoot, int maxDepth);
...
public:
	...
	Optional<DataObjectComponentReference> dataObjectComponent;

	Optional<unsigned int> containerType;

	Optional<unsigned int> containerVolumeLimit;

	Optional<ContainerPermissions> containerPermissions;

	Optional<unsigned int> serverObjectCRC;

	Optional<unsigned int> clientObjectCRC;

	Optional<Quaternion> direction;

	Optional<unsigned int> movementCounter;

	Optional<StringId> objectName;

	Optional<bool> forceSend;

	Optional<bool> staticObject;

	Optional<VectorMap<unsigned int, ManagedReference<FacadePOD* > >> objectActiveSessions;

	Optional<unsigned int> gameObjectType;

	Optional<unsigned int> containmentType;
	...
	Optional<SortedVector<ManagedReference<SceneObjectPOD* > >> childObjects;
