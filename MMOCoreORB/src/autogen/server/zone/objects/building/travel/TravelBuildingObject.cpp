/*
 *	autogen/server/zone/objects/building/travel/TravelBuildingObject.cpp generated by engine3 IDL compiler 0.70
 */
...
DistributedObjectServant* TravelBuildingObject::_getImplementationForRead() const {
	return _impl;
}
...
void TravelBuildingObjectImplementation::readObject(ObjectInputStream* stream) {
	...
	for (int i = 0; i < _varCount; ++i) {
		...
		if(TravelBuildingObjectImplementation::readObjectMember(stream, _nameHashCode)) {
		}
...
bool TravelBuildingObjectImplementation::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void TravelBuildingObjectImplementation::writeJSON(nlohmann::json& j) {
...
DistributedObjectPOD* TravelBuildingObjectHelper::instantiatePOD() {
	return new TravelBuildingObjectPOD();
}
...
TravelBuildingObjectPOD::~TravelBuildingObjectPOD() {
	finalize();
}

TravelBuildingObjectPOD::TravelBuildingObjectPOD(void) {
	_className = "TravelBuildingObject";
}


void TravelBuildingObjectPOD::writeJSON(nlohmann::json& j) {
...
void TravelBuildingObjectPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = TravelBuildingObjectPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int TravelBuildingObjectPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool TravelBuildingObjectPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void TravelBuildingObjectPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(TravelBuildingObjectPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void TravelBuildingObjectPOD::writeObjectCompact(ObjectOutputStream* stream) {
