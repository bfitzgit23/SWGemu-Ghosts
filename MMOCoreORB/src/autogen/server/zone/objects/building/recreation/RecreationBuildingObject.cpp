/*
 *	autogen/server/zone/objects/building/recreation/RecreationBuildingObject.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_ISRECREATIONALBUILDINGOBJECT__ = 2450418991};
...
DistributedObjectServant* RecreationBuildingObject::_getImplementationForRead() const {
	return _impl;
}
...
bool RecreationBuildingObjectImplementation::isRecreationalBuildingObject() const{
	...
	case RPC_ISRECREATIONALBUILDINGOBJECT__:
		{
			...
			bool _m_res = isRecreationalBuildingObject();
...
bool RecreationBuildingObjectAdapter::isRecreationalBuildingObject() const {
...
DistributedObjectPOD* RecreationBuildingObjectHelper::instantiatePOD() {
	return new RecreationBuildingObjectPOD();
}
...
RecreationBuildingObjectPOD::~RecreationBuildingObjectPOD() {
	finalize();
}

RecreationBuildingObjectPOD::RecreationBuildingObjectPOD(void) {
	_className = "RecreationBuildingObject";
}


void RecreationBuildingObjectPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = RecreationBuildingObjectPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int RecreationBuildingObjectPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool RecreationBuildingObjectPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void RecreationBuildingObjectPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(RecreationBuildingObjectPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void RecreationBuildingObjectPOD::writeObjectCompact(ObjectOutputStream* stream) {
