/*
 *	autogen/server/zone/objects/tangible/tool/smuggler/SlicingTool.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_HANDLEOBJECTMENUSELECT__CREATUREOBJECT_BYTE_ = 980607666,RPC_CALCULATESUCCESSRATE__,RPC_GETEFFECTIVENESS__};
...
DistributedObjectServant* SlicingTool::_getImplementationForRead() const {
	return _impl;
}
	...
	case RPC_CALCULATESUCCESSRATE__:
		{
			...
			bool _m_res = calculateSuccessRate();
...
DistributedObjectPOD* SlicingToolHelper::instantiatePOD() {
	return new SlicingToolPOD();
}
...
SlicingToolPOD::~SlicingToolPOD() {
	finalize();
}

SlicingToolPOD::SlicingToolPOD(void) {
	_className = "SlicingTool";
}


void SlicingToolPOD::writeJSON(nlohmann::json& j) {
...
void SlicingToolPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = SlicingToolPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int SlicingToolPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool SlicingToolPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void SlicingToolPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(SlicingToolPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void SlicingToolPOD::writeObjectCompact(ObjectOutputStream* stream) {
