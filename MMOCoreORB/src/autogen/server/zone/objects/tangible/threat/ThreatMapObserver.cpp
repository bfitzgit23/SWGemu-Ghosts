/*
 *	autogen/server/zone/objects/tangible/threat/ThreatMapObserver.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_NOTIFYOBSERVEREVENT__INT_OBSERVABLE_MANAGEDOBJECT_LONG_ = 14256476};
...
DistributedObjectServant* ThreatMapObserver::_getImplementationForRead() const {
	return _impl;
}
...
DistributedObjectPOD* ThreatMapObserverHelper::instantiatePOD() {
	return new ThreatMapObserverPOD();
}
...
ThreatMapObserverPOD::~ThreatMapObserverPOD() {
}

ThreatMapObserverPOD::ThreatMapObserverPOD(void) {
	_className = "ThreatMapObserver";
}


void ThreatMapObserverPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = ThreatMapObserverPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int ThreatMapObserverPOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<ManagedWeakReference<TangibleObjectPOD* > >::toBinaryStream(&self.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool ThreatMapObserverPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0x6f6cc7f8: //ThreatMapObserver.self
		{
			ManagedWeakReference<TangibleObjectPOD* > _mnself;
			TypeInfo<ManagedWeakReference<TangibleObjectPOD* > >::parseFromBinaryStream(&_mnself, stream);
			self = std::move(_mnself);
		}
		return true;
...
void ThreatMapObserverPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(ThreatMapObserverPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void ThreatMapObserverPOD::writeObjectCompact(ObjectOutputStream* stream) {
