/*
 *	autogen/server/zone/objects/tangible/attachment/Attachment.cpp generated by engine3 IDL compiler 0.70
 */
...
DistributedObjectServant* Attachment::_getImplementationForRead() const {
	return _impl;
}
...
void AttachmentImplementation::writeJSON(nlohmann::json& j) {
	TangibleObjectImplementation::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	thisObject["attachmentType"] = attachmentType;
	...
	case RPC_ISCLOTHINGATTACHMENT__:
		{
			...
			bool _m_res = isClothingAttachment();
			resp->insertBoolean(_m_res);
		}
		break;
	default:
...
DistributedObjectPOD* AttachmentHelper::instantiatePOD() {
	return new AttachmentPOD();
}
...
AttachmentPOD::AttachmentPOD(void) {
	_className = "Attachment";
}


void AttachmentPOD::writeJSON(nlohmann::json& j) {
	TangibleObjectPOD::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	if (attachmentType)
		thisObject["attachmentType"] = attachmentType.value();
...
void AttachmentPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = AttachmentPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int AttachmentPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool AttachmentPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0x98b91e81: //Attachment.attachmentType
		{
			int _mnattachmentType;
			TypeInfo<int >::parseFromBinaryStream(&_mnattachmentType, stream);
			attachmentType = std::move(_mnattachmentType);
		}
		return true;
...
void AttachmentPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(AttachmentPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void AttachmentPOD::writeObjectCompact(ObjectOutputStream* stream) {
