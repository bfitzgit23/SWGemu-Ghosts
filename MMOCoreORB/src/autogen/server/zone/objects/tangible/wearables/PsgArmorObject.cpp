/*
 *	autogen/server/zone/objects/tangible/wearables/PsgArmorObject.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_INITIALIZETRANSIENTMEMBERS__ = 3719141856,RPC_ISPSGARMOROBJECT__,};
...
DistributedObjectServant* PsgArmorObject::_getImplementationForRead() const {
	return _impl;
}
...
void PsgArmorObjectImplementation::writeJSON(nlohmann::json& j) {
	ArmorObjectImplementation::writeJSON(j);
...
void PsgArmorObjectAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_ISPSGARMOROBJECT__:
		...
		break;
	default:
		ArmorObjectAdapter::invokeMethod(methid, inv);
	}
}
...
DistributedObjectPOD* PsgArmorObjectHelper::instantiatePOD() {
	return new PsgArmorObjectPOD();
}
...
PsgArmorObjectPOD::~PsgArmorObjectPOD() {
	finalize();
}

PsgArmorObjectPOD::PsgArmorObjectPOD(void) {
	_className = "PsgArmorObject";
}


void PsgArmorObjectPOD::writeJSON(nlohmann::json& j) {
	ArmorObjectPOD::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
}


void PsgArmorObjectPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = PsgArmorObjectPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int PsgArmorObjectPOD::writeObjectMembers(ObjectOutputStream* stream) {
	int _count = ArmorObjectPOD::writeObjectMembers(stream);
...
bool PsgArmorObjectPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	if (ArmorObjectPOD::readObjectMember(stream, nameHashCode))
		return true;
...
void PsgArmorObjectPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(PsgArmorObjectPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void PsgArmorObjectPOD::writeObjectCompact(ObjectOutputStream* stream) {
	ArmorObjectPOD::writeObjectCompact(stream);
