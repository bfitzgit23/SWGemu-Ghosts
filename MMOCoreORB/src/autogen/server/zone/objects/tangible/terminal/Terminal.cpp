/*
 *	autogen/server/zone/objects/tangible/terminal/Terminal.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_INITIALIZETRANSIENTMEMBERS__ = 2743824535,RPC_ISTERMINAL__,RPC_ISGUILDTERMINAL__,RPC_SETCONTROLLEDOBJECT__SCENEOBJECT_,RPC_GETCONTROLLEDOBJECT__,RPC_ISELEVATORTERMINAL__,RPC_ISVENDORTERMINAL__,RPC_ISBAZAARTERMINAL__,RPC_INITIALIZECHILDOBJECT__SCENEOBJECT_};
...
DistributedObjectServant* Terminal::_getImplementationForRead() const {
	return _impl;
}
	...
	case RPC_ISELEVATORTERMINAL__:
		{
			...
			bool _m_res = isElevatorTerminal();
			resp->insertBoolean(_m_res);
		}
		break;
...
DistributedObjectPOD* TerminalHelper::instantiatePOD() {
	return new TerminalPOD();
}
...
TerminalPOD::TerminalPOD(void) {
	_className = "Terminal";
}


void TerminalPOD::writeJSON(nlohmann::json& j) {
...
void TerminalPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = TerminalPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int TerminalPOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<ManagedReference<SceneObjectPOD* > >::toBinaryStream(&controlledObject.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool TerminalPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0x33c4eb68: //Terminal.controlledObject
		{
			ManagedReference<SceneObjectPOD* > _mncontrolledObject;
			TypeInfo<ManagedReference<SceneObjectPOD* > >::parseFromBinaryStream(&_mncontrolledObject, stream);
...
void TerminalPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(TerminalPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void TerminalPOD::writeObjectCompact(ObjectOutputStream* stream) {
