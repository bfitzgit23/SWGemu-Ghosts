/*
 *	autogen/server/zone/objects/tangible/terminal/mission/MissionTerminal.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_INITIALIZETRANSIENTMEMBERS__,RPC_HANDLEOBJECTMENUSELECT__CREATUREOBJECT_BYTE_,RPC_GETTERMINALNAME__,RPC_ISMISSIONTERMINAL__,RPC_ISARTISANTERMINAL__,RPC_ISGENERALTERMINAL__,RPC_ISBOUNTYTERMINAL__,RPC_ISENTERTAINERTERMINAL__,RPC_ISIMPERIALTERMINAL__,RPC_ISNEWBIETERMINAL__,RPC_ISREBELTERMINAL__,RPC_ISSCOUTTERMINAL__,RPC_ISSTATUETERMINAL__,RPC_ISSLICER__CREATUREOBJECT_,RPC_ADDSLICER__CREATUREOBJECT_,RPC_REMOVESLICER__CREATUREOBJECT_};
...
DistributedObjectServant* MissionTerminal::_getImplementationForRead() const {
	return _impl;
}
...
void MissionTerminalImplementation::writeJSON(nlohmann::json& j) {
	TerminalImplementation::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	thisObject["terminalType"] = terminalType;

	j["MissionTerminal"] = thisObject;
}
...
void MissionTerminalImplementation::loadTemplateData(SharedObjectTemplate* templateData) {
	...
	MissionTerminalTemplate* templ = dynamic_cast<MissionTerminalTemplate*>(templateData);
...
void MissionTerminalImplementation::addSlicer(CreatureObject* slicer) {
	// server/zone/objects/tangible/terminal/mission/MissionTerminal.idl():  	}
	if (!(&slicers)->contains(slicer))	// server/zone/objects/tangible/terminal/mission/MissionTerminal.idl():  			slicers.add(slicer);
	(&slicers)->add(slicer);
}
	...
	case RPC_ISENTERTAINERTERMINAL__:
		{
			...
			bool _m_res = isEntertainerTerminal();
			resp->insertBoolean(_m_res);
		}
		break;
	case RPC_ISIMPERIALTERMINAL__:
		{
			...
			bool _m_res = isImperialTerminal();
			resp->insertBoolean(_m_res);
		}
		break;
...
DistributedObjectPOD* MissionTerminalHelper::instantiatePOD() {
	return new MissionTerminalPOD();
}
...
MissionTerminalPOD::~MissionTerminalPOD() {
	finalize();
}

MissionTerminalPOD::MissionTerminalPOD(void) {
	_className = "MissionTerminal";
}


void MissionTerminalPOD::writeJSON(nlohmann::json& j) {
	TerminalPOD::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	if (terminalType)
		thisObject["terminalType"] = terminalType.value();

	j["MissionTerminal"] = thisObject;
}


void MissionTerminalPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = MissionTerminalPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int MissionTerminalPOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<String >::toBinaryStream(&terminalType.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool MissionTerminalPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0x12e9a7e: //MissionTerminal.terminalType
		{
			String _mnterminalType;
			TypeInfo<String >::parseFromBinaryStream(&_mnterminalType, stream);
			terminalType = std::move(_mnterminalType);
		}
		return true;
...
void MissionTerminalPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(MissionTerminalPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void MissionTerminalPOD::writeObjectCompact(ObjectOutputStream* stream) {
