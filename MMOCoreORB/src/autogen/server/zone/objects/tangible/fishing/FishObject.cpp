/*
 *	autogen/server/zone/objects/tangible/fishing/FishObject.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_INITIALIZETRANSIENTMEMBERS__ = 1950784664,RPC_SETATTRIBUTES__STRING_STRING_STRING_FLOAT_,RPC_HANDLEOBJECTMENUSELECT__CREATUREOBJECT_BYTE_,RPC_FILET__CREATUREOBJECT_};
...
DistributedObjectServant* FishObject::_getImplementationForRead() const {
	return _impl;
}
...
void FishObjectImplementation::writeJSON(nlohmann::json& j) {
	TangibleObjectImplementation::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	thisObject["player"] = player;

	thisObject["zoneName"] = zoneName;

	thisObject["timeCaught"] = timeCaught;

	thisObject["length"] = length;

	j["FishObject"] = thisObject;
}
	...
	case RPC_SETATTRIBUTES__STRING_STRING_STRING_FLOAT_:
		{
			 ...
			 String terrainN; inv->getAsciiParameter(terrainN);
			String timestamp; inv->getAsciiParameter(timestamp);
			float fishLength = inv->getFloatParameter();
			
			setAttributes(playerName, terrainN, timestamp, fishLength);
...
DistributedObjectPOD* FishObjectHelper::instantiatePOD() {
	return new FishObjectPOD();
}
...
FishObjectPOD::~FishObjectPOD() {
	finalize();
}

FishObjectPOD::FishObjectPOD(void) {
	_className = "FishObject";
}


void FishObjectPOD::writeJSON(nlohmann::json& j) {
	...
	if (zoneName)
		thisObject["zoneName"] = zoneName.value();

	if (timeCaught)
		thisObject["timeCaught"] = timeCaught.value();

	if (length)
		thisObject["length"] = length.value();

	j["FishObject"] = thisObject;
}


void FishObjectPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = FishObjectPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int FishObjectPOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<String >::toBinaryStream(&player.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<String >::toBinaryStream(&timeCaught.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool FishObjectPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0xe1441c39: //FishObject.player
		{
			String _mnplayer;
			TypeInfo<String >::parseFromBinaryStream(&_mnplayer, stream);
	...
	case 0x59b010dd: //FishObject.timeCaught
		{
			String _mntimeCaught;
			TypeInfo<String >::parseFromBinaryStream(&_mntimeCaught, stream);
			timeCaught = std::move(_mntimeCaught);
		}
		return true;
...
void FishObjectPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(FishObjectPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void FishObjectPOD::writeObjectCompact(ObjectOutputStream* stream) {
