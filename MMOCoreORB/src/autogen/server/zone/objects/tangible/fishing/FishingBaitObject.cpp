/*
 *	autogen/server/zone/objects/tangible/fishing/FishingBaitObject.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_INITIALIZETRANSIENTMEMBERS__ = 864827427,RPC_GETFRESHNESS__,RPC_SETFRESHNESS__INT_,RPC_LESSFRESH__,RPC_GETUSECOUNT__,};
...
DistributedObjectServant* FishingBaitObject::_getImplementationForRead() const {
	return _impl;
}
...
void FishingBaitObjectImplementation::writeJSON(nlohmann::json& j) {
	TangibleObjectImplementation::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	thisObject["freshness"] = freshness;

	j["FishingBaitObject"] = thisObject;
}
...
int FishingBaitObjectImplementation::getFreshness() const{
...
int FishingBaitObjectAdapter::getFreshness() const {
...
int FishingBaitObjectAdapter::getUseCount() const {
...
DistributedObjectPOD* FishingBaitObjectHelper::instantiatePOD() {
	return new FishingBaitObjectPOD();
}
...
FishingBaitObjectPOD::~FishingBaitObjectPOD() {
	finalize();
}

FishingBaitObjectPOD::FishingBaitObjectPOD(void) {
	_className = "FishingBaitObject";
}


void FishingBaitObjectPOD::writeJSON(nlohmann::json& j) {
	TangibleObjectPOD::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	if (freshness)
		thisObject["freshness"] = freshness.value();

	j["FishingBaitObject"] = thisObject;
}


void FishingBaitObjectPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = FishingBaitObjectPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int FishingBaitObjectPOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<int >::toBinaryStream(&freshness.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool FishingBaitObjectPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0xe0f2fb8: //FishingBaitObject.freshness
		{
			int _mnfreshness;
			TypeInfo<int >::parseFromBinaryStream(&_mnfreshness, stream);
			freshness = std::move(_mnfreshness);
		}
		return true;
...
void FishingBaitObjectPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(FishingBaitObjectPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void FishingBaitObjectPOD::writeObjectCompact(ObjectOutputStream* stream) {
