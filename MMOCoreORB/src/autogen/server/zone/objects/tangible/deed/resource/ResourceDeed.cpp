/*
 *	autogen/server/zone/objects/tangible/deed/resource/ResourceDeed.cpp generated by engine3 IDL compiler 0.70
 */
...
void ResourceDeedImplementation::writeJSON(nlohmann::json& j) {
...
DistributedObjectPOD* ResourceDeedHelper::instantiatePOD() {
	return new ResourceDeedPOD();
}
...
ResourceDeedPOD::~ResourceDeedPOD() {
	finalize();
}

ResourceDeedPOD::ResourceDeedPOD(void) {
	_className = "ResourceDeed";
}


void ResourceDeedPOD::writeJSON(nlohmann::json& j) {
	DeedPOD::writeJSON(j);
...
void ResourceDeedPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = ResourceDeedPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int ResourceDeedPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool ResourceDeedPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void ResourceDeedPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(ResourceDeedPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void ResourceDeedPOD::writeObjectCompact(ObjectOutputStream* stream) {
