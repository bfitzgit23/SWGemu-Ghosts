/*
 *	autogen/server/zone/objects/tangible/consumable/DelayedBuffObserver.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_NOTIFYOBSERVEREVENT__INT_OBSERVABLE_MANAGEDOBJECT_LONG_ = 1712113196};
...
DistributedObjectServant* DelayedBuffObserver::_getImplementationForRead() const {
	return _impl;
}
...
void DelayedBuffObserverImplementation::writeJSON(nlohmann::json& j) {
	ObserverImplementation::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	thisObject["buff"] = buff;

	j["DelayedBuffObserver"] = thisObject;
}
...
DistributedObjectPOD* DelayedBuffObserverHelper::instantiatePOD() {
	return new DelayedBuffObserverPOD();
}
...
DelayedBuffObserverPOD::~DelayedBuffObserverPOD() {
}

DelayedBuffObserverPOD::DelayedBuffObserverPOD(void) {
	_className = "DelayedBuffObserver";
}


void DelayedBuffObserverPOD::writeJSON(nlohmann::json& j) {
...
void DelayedBuffObserverPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = DelayedBuffObserverPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int DelayedBuffObserverPOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<ManagedWeakReference<DelayedBuffPOD* > >::toBinaryStream(&buff.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool DelayedBuffObserverPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0xe84cbee4: //DelayedBuffObserver.buff
		{
			ManagedWeakReference<DelayedBuffPOD* > _mnbuff;
			TypeInfo<ManagedWeakReference<DelayedBuffPOD* > >::parseFromBinaryStream(&_mnbuff, stream);
...
void DelayedBuffObserverPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(DelayedBuffObserverPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void DelayedBuffObserverPOD::writeObjectCompact(ObjectOutputStream* stream) {
