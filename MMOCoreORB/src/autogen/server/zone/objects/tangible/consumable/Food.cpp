/*
 *	autogen/server/zone/objects/tangible/consumable/Food.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_INITIALIZETRANSIENTMEMBERS__ = 204772233,RPC_INITIALIZEPRIVATEDATA__};
...
DistributedObjectServant* Food::_getImplementationForRead() const {
	return _impl;
}
...
void FoodImplementation::writeJSON(nlohmann::json& j) {
...
DistributedObjectPOD* FoodHelper::instantiatePOD() {
	return new FoodPOD();
}
...
FoodPOD::FoodPOD(void) {
	_className = "Food";
}


void FoodPOD::writeJSON(nlohmann::json& j) {
...
void FoodPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = FoodPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int FoodPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool FoodPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void FoodPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(FoodPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void FoodPOD::writeObjectCompact(ObjectOutputStream* stream) {
