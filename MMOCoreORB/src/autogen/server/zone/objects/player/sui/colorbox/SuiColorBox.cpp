/*
 *	autogen/server/zone/objects/player/sui/colorbox/SuiColorBox.cpp generated by engine3 IDL compiler 0.70
 */
...
DistributedObjectServant* SuiColorBox::_getImplementationForRead() const {
	return _impl;
}
	...
	case RPC_GETCOLORPALETTE__:
		{
			...
			String _m_res = getColorPalette();
			resp->insertAscii(_m_res);
		}
		break;
	case RPC_SETCOLORPALETTE__STRING_:
		{
			String pal; inv->getAsciiParameter(pal);
...
DistributedObjectPOD* SuiColorBoxHelper::instantiatePOD() {
	return new SuiColorBoxPOD();
}
...
SuiColorBoxPOD::~SuiColorBoxPOD() {
	finalize();
}

SuiColorBoxPOD::SuiColorBoxPOD(void) {
	_className = "SuiColorBox";
}


void SuiColorBoxPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = SuiColorBoxPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int SuiColorBoxPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool SuiColorBoxPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void SuiColorBoxPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(SuiColorBoxPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void SuiColorBoxPOD::writeObjectCompact(ObjectOutputStream* stream) {
