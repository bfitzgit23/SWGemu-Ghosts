/*
 *	autogen/server/zone/objects/player/sui/listbox/SuiListBox.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_INIT__ = 1325352454,RPC_ADDMENUITEM__STRING_LONG_,RPC_GETMENUITEMNAME__INT_,RPC_REMOVEALLMENUITEMS__,RPC_GETMENUOBJECTID__INT_,RPC_GETMENUSIZE__,RPC_SETNEXTBOX__INT_,RPC_SETPREVIOUSBOX__INT_,RPC_GETNEXTBOX__,RPC_GETPREVIOUSBOX__,RPC_ISLISTBOX__};
...
DistributedObjectServant* SuiListBox::_getImplementationForRead() const {
	return _impl;
}
	...
	case RPC_GETMENUITEMNAME__INT_:
		{
			...
			String _m_res = getMenuItemName(index);
	...
	case RPC_GETMENUOBJECTID__INT_:
		{
			...
			unsigned long long _m_res = getMenuObjectID(idx);
	...
	case RPC_GETNEXTBOX__:
		{
			...
			unsigned long long _m_res = getNextBox();
			resp->insertLong(_m_res);
		}
		break;
	case RPC_GETPREVIOUSBOX__:
		{
			...
			unsigned long long _m_res = getPreviousBox();
			resp->insertLong(_m_res);
		}
		break;
...
DistributedObjectPOD* SuiListBoxHelper::instantiatePOD() {
	return new SuiListBoxPOD();
}
...
SuiListBoxPOD::~SuiListBoxPOD() {
	finalize();
}

SuiListBoxPOD::SuiListBoxPOD(void) {
	_className = "SuiListBox";
}


void SuiListBoxPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = SuiListBoxPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int SuiListBoxPOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<Vector<ManagedReference<SuiListBoxMenuItemPOD* > > >::toBinaryStream(&menuItems.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<unsigned long long >::toBinaryStream(&next.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<unsigned long long >::toBinaryStream(&previous.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool SuiListBoxPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0x376a33bd: //SuiListBox.menuItems
		{
			Vector<ManagedReference<SuiListBoxMenuItemPOD* > > _mnmenuItems;
			TypeInfo<Vector<ManagedReference<SuiListBoxMenuItemPOD* > > >::parseFromBinaryStream(&_mnmenuItems, stream);
			menuItems = std::move(_mnmenuItems);
		}
		return true;
	...
	case 0xe439ff44: //SuiListBox.next
		{
			unsigned long long _mnnext;
			TypeInfo<unsigned long long >::parseFromBinaryStream(&_mnnext, stream);
			next = std::move(_mnnext);
		}
		return true;

	case 0xea8b6254: //SuiListBox.previous
		{
			unsigned long long _mnprevious;
			TypeInfo<unsigned long long >::parseFromBinaryStream(&_mnprevious, stream);
			previous = std::move(_mnprevious);
		}
		return true;
...
void SuiListBoxPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(SuiListBoxPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void SuiListBoxPOD::writeObjectCompact(ObjectOutputStream* stream) {
