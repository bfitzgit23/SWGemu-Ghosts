/*
 *	autogen/server/zone/objects/player/sui/characterbuilderbox/SuiCharacterBuilderBox.cpp generated by engine3 IDL compiler 0.70
 */
...
DistributedObjectServant* SuiCharacterBuilderBox::_getImplementationForRead() const {
	return _impl;
}
...
DistributedObjectPOD* SuiCharacterBuilderBoxHelper::instantiatePOD() {
	return new SuiCharacterBuilderBoxPOD();
}
...
SuiCharacterBuilderBoxPOD::~SuiCharacterBuilderBoxPOD() {
	finalize();
}

SuiCharacterBuilderBoxPOD::SuiCharacterBuilderBoxPOD(void) {
	_className = "SuiCharacterBuilderBox";
}


void SuiCharacterBuilderBoxPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = SuiCharacterBuilderBoxPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int SuiCharacterBuilderBoxPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool SuiCharacterBuilderBoxPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void SuiCharacterBuilderBoxPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(SuiCharacterBuilderBoxPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void SuiCharacterBuilderBoxPOD::writeObjectCompact(ObjectOutputStream* stream) {
