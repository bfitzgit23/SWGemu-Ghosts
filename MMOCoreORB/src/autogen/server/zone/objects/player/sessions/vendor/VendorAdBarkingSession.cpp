/*
 *	autogen/server/zone/objects/player/sessions/vendor/VendorAdBarkingSession.cpp generated by engine3 IDL compiler 0.70
 */
...
DistributedObjectServant* VendorAdBarkingSession::_getImplementationForRead() const {
	return _impl;
}
...
int VendorAdBarkingSessionImplementation::cancelSession() {
	// server/zone/objects/player/sessions/vendor/VendorAdBarkingSession.idl():  		CreatureObject player = owner;
	ManagedReference<CreatureObject* > player = owner;
	// server/zone/objects/player/sessions/vendor/VendorAdBarkingSession.idl():  		clearSession(
	if (player)	// server/zone/objects/player/sessions/vendor/VendorAdBarkingSession.idl():  			player.dropActiveSession(SessionFacadeType.VENDORADBARKING);
...
void VendorAdBarkingSessionAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_SETMOOD__STRING_:
		{
			 String mo; inv->getAsciiParameter(mo);
	...
	case RPC_SETANIMATION__STRING_:
		{
			 String ani; inv->getAsciiParameter(ani);
...
DistributedObjectPOD* VendorAdBarkingSessionHelper::instantiatePOD() {
	return new VendorAdBarkingSessionPOD();
}
...
VendorAdBarkingSessionPOD::~VendorAdBarkingSessionPOD() {
}

VendorAdBarkingSessionPOD::VendorAdBarkingSessionPOD(void) {
	_className = "VendorAdBarkingSession";
}


void VendorAdBarkingSessionPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = VendorAdBarkingSessionPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int VendorAdBarkingSessionPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool VendorAdBarkingSessionPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0xfdc5b7b0: //VendorAdBarkingSession.advertisingMod
		{
			int _mnadvertisingMod;
			TypeInfo<int >::parseFromBinaryStream(&_mnadvertisingMod, stream);
			advertisingMod = std::move(_mnadvertisingMod);
		}
		return true;
...
void VendorAdBarkingSessionPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(VendorAdBarkingSessionPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void VendorAdBarkingSessionPOD::writeObjectCompact(ObjectOutputStream* stream) {
