...
int CraftingSession::cancelSessionCommand() {
	...
	if (unlikely(_implementation == NULL)) {
		...
		DistributedMethod method(this, RPC_CANCELSESSIONCOMMAND__);

		return method.executeWithSignedIntReturn();
	} else {
		return _implementation->cancelSessionCommand();
	}
}
...
void CraftingSessionAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_CANCELSESSION__:
		...
		break;
	case RPC_CANCELSESSIONCOMMAND__:
		{
			
			int _m_res = cancelSessionCommand();
			resp->insertSignedInt(_m_res);
		}
		break;
...
int CraftingSessionAdapter::cancelSessionCommand() {
	return (static_cast<CraftingSession*>(stub))->cancelSessionCommand();
}
