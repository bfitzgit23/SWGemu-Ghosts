/*
 *	autogen/server/zone/objects/player/sessions/DestroyStructureSession.cpp generated by engine3 IDL compiler 0.70
 */
...
DistributedObjectPOD* DestroyStructureSessionHelper::instantiatePOD() {
	return new DestroyStructureSessionPOD();
}
...
/*
 *	DestroyStructureSessionPOD
 */

DestroyStructureSessionPOD::~DestroyStructureSessionPOD() {
}

DestroyStructureSessionPOD::DestroyStructureSessionPOD(void) {
	_className = "DestroyStructureSession";
}


void DestroyStructureSessionPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = DestroyStructureSessionPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int DestroyStructureSessionPOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<unsigned int >::toBinaryStream(&destroyCode.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool DestroyStructureSessionPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0xe4f1440a: //DestroyStructureSession.destroyCode
		{
			unsigned int _mndestroyCode;
			TypeInfo<unsigned int >::parseFromBinaryStream(&_mndestroyCode, stream);
			destroyCode = std::move(_mndestroyCode);
		}
		return true;
...
void DestroyStructureSessionPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(DestroyStructureSessionPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void DestroyStructureSessionPOD::writeObjectCompact(ObjectOutputStream* stream) {
