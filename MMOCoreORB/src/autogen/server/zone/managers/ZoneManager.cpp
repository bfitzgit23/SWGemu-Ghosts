/*
 *	autogen/server/zone/managers/ZoneManager.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_SETZONEPROCESSOR__ZONEPROCESSSERVER_ = 390616416};
...
DistributedObjectServant* ZoneManager::_getImplementationForRead() const {
	return _impl;
}
...
void ZoneManagerImplementation::readObject(ObjectInputStream* stream) {
	...
	for (int i = 0; i < _varCount; ++i) {
		...
		if(ZoneManagerImplementation::readObjectMember(stream, _nameHashCode)) {
		}
...
bool ZoneManagerImplementation::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void ZoneManagerAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_SETZONEPROCESSOR__ZONEPROCESSSERVER_:
		{
			ZoneProcessServer* server = static_cast<ZoneProcessServer*>(inv->getObjectParameter());
			
			setZoneProcessor(server);
...
DistributedObjectPOD* ZoneManagerHelper::instantiatePOD() {
	return new ZoneManagerPOD();
}
...
ZoneManagerPOD::~ZoneManagerPOD() {
}

ZoneManagerPOD::ZoneManagerPOD(void) {
	_className = "ZoneManager";
}


void ZoneManagerPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = ZoneManagerPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int ZoneManagerPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool ZoneManagerPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void ZoneManagerPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(ZoneManagerPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void ZoneManagerPOD::writeObjectCompact(ObjectOutputStream* stream) {
