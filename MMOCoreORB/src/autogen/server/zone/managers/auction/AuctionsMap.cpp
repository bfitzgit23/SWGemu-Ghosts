/*
 *	autogen/server/zone/managers/auction/AuctionsMap.cpp generated by engine3 IDL compiler 0.70
 */
...
void AuctionsMapImplementation::writeJSON(nlohmann::json& j) {
	ManagedObjectImplementation::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	thisObject["commoditiesLimit"] = commoditiesLimit;

	j["AuctionsMap"] = thisObject;
}
...
int AuctionsMapAdapter::getVendorCount() const {
	return (static_cast<AuctionsMap*>(stub))->getVendorCount();
}

int AuctionsMapAdapter::getTotalItemCount() const {
...
DistributedObjectPOD* AuctionsMapHelper::instantiatePOD() {
	return new AuctionsMapPOD();
}
...
AuctionsMapPOD::~AuctionsMapPOD() {
}

AuctionsMapPOD::AuctionsMapPOD(void) {
	_className = "AuctionsMap";
}


void AuctionsMapPOD::writeJSON(nlohmann::json& j) {
	ManagedObjectPOD::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	if (commoditiesLimit)
		thisObject["commoditiesLimit"] = commoditiesLimit.value();

	j["AuctionsMap"] = thisObject;
}


void AuctionsMapPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = AuctionsMapPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int AuctionsMapPOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<CommoditiesLimit >::toBinaryStream(&commoditiesLimit.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool AuctionsMapPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0xbb9b7ce5: //AuctionsMap.commoditiesLimit
		{
			CommoditiesLimit _mncommoditiesLimit;
			TypeInfo<CommoditiesLimit >::parseFromBinaryStream(&_mncommoditiesLimit, stream);
			commoditiesLimit = std::move(_mncommoditiesLimit);
		}
		return true;
...
void AuctionsMapPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(AuctionsMapPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void AuctionsMapPOD::writeObjectCompact(ObjectOutputStream* stream) {
