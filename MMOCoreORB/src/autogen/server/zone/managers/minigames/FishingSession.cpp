/*
 *	autogen/server/zone/managers/minigames/FishingSession.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_SETMOOD__STRING_,RPC_SETNEXTACTION__INT_,RPC_GETNEXTACTION__,RPC_SETFISHBOXID__INT_,RPC_GETFISHBOXID__,RPC_GETMARKER__,RPC_SETMARKER__SCENEOBJECT_,RPC_GETFISH__,RPC_SETFISH__INT_,RPC_GETFISHINGSTATE__,RPC_SETFISHINGSTATE__INT_,RPC_UPDATE__INT_SCENEOBJECT_INT_INT_INT_,RPC_GETMOODSTRING__};

FishingSession::FishingSession(FishingEvent* event, SceneObject* marker, int nextAction, int fish, unsigned int boxID, int fishingState, const String& mood) : Facade(DummyConstructorParameter::instance()) {
	FishingSessionImplementation* _implementation = new FishingSessionImplementation(event, marker, nextAction, fish, boxID, fishingState, mood);
...
void FishingSessionImplementation::writeJSON(nlohmann::json& j) {
	FacadeImplementation::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	thisObject["marker"] = marker;

	thisObject["nextAction"] = nextAction;

	thisObject["fish"] = fish;

	thisObject["boxID"] = boxID;

	thisObject["fishingState"] = fishingState;

	thisObject["mood"] = mood;

	j["FishingSession"] = thisObject;
}

FishingSessionImplementation::FishingSessionImplementation(FishingEvent* event, SceneObject* marker, int nextAction, int fish, unsigned int boxID, int fishingState, const String& mood) {
	_initializeImplementation();
...
DistributedObjectPOD* FishingSessionHelper::instantiatePOD() {
	return new FishingSessionPOD();
}
...
FishingSessionPOD::~FishingSessionPOD() {
}

FishingSessionPOD::FishingSessionPOD(void) {
	_className = "FishingSession";
}


void FishingSessionPOD::writeJSON(nlohmann::json& j) {
	FacadePOD::writeJSON(j);

	nlohmann::json thisObject = nlohmann::json::object();
	if (marker)
		thisObject["marker"] = marker.value();

	if (nextAction)
		thisObject["nextAction"] = nextAction.value();

	if (fish)
		thisObject["fish"] = fish.value();

	if (boxID)
		thisObject["boxID"] = boxID.value();

	if (fishingState)
		thisObject["fishingState"] = fishingState.value();

	if (mood)
		thisObject["mood"] = mood.value();

	j["FishingSession"] = thisObject;
}


void FishingSessionPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = FishingSessionPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int FishingSessionPOD::writeObjectMembers(ObjectOutputStream* stream) {
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<ManagedReference<SceneObjectPOD* > >::toBinaryStream(&marker.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<int >::toBinaryStream(&nextAction.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<int >::toBinaryStream(&fish.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<int >::toBinaryStream(&fishingState.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
	...
	_offset = stream->getOffset();
	stream->writeInt(0);
	TypeInfo<String >::toBinaryStream(&mood.value(), stream);
	_totalSize = (uint32) (stream->getOffset() - (_offset + 4));
	stream->writeInt(_offset, _totalSize);
	_count++;
	}
...
bool FishingSessionPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
	...
	case 0x53cdfb9b: //FishingSession.marker
		{
			ManagedReference<SceneObjectPOD* > _mnmarker;
			TypeInfo<ManagedReference<SceneObjectPOD* > >::parseFromBinaryStream(&_mnmarker, stream);
			marker = std::move(_mnmarker);
		}
		return true;
	...
	case 0xc7d84585: //FishingSession.nextAction
		{
			int _mnnextAction;
			TypeInfo<int >::parseFromBinaryStream(&_mnnextAction, stream);
	...
	case 0xeb753405: //FishingSession.fish
		{
			int _mnfish;
			TypeInfo<int >::parseFromBinaryStream(&_mnfish, stream);
			fish = std::move(_mnfish);
		}
		return true;
	...
	case 0x134df457: //FishingSession.fishingState
		{
			int _mnfishingState;
			TypeInfo<int >::parseFromBinaryStream(&_mnfishingState, stream);
			fishingState = std::move(_mnfishingState);
		}
		return true;
	...
	case 0x9c0a97ff: //FishingSession.mood
		{
			String _mnmood;
			TypeInfo<String >::parseFromBinaryStream(&_mnmood, stream);
			mood = std::move(_mnmood);
		}
		return true;
...
void FishingSessionPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(FishingSessionPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void FishingSessionPOD::writeObjectCompact(ObjectOutputStream* stream) {
