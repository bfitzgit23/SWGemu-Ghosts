/*
 *	autogen/server/zone/managers/crafting/CraftingManager.cpp generated by engine3 IDL compiler 0.70
 */
...
enum {RPC_STOP__,RPC_GETSCHEMATIC__INT_,RPC_SENDDRAFTSLOTSTO__CREATUREOBJECT_INT_,RPC_SENDRESOURCEWEIGHTSTO__CREATUREOBJECT_INT_,RPC_CALCULATEASSEMBLYSUCCESS__CREATUREOBJECT_DRAFTSCHEMATIC_FLOAT_,RPC_CALCULATEEXPERIMENTATIONFAILURERATE__CREATUREOBJECT_MANUFACTURESCHEMATIC_INT_,RPC_CALCULATEEXPERIMENTATIONSUCCESS__CREATUREOBJECT_DRAFTSCHEMATIC_FLOAT_,RPC_GENERATESERIAL__,RPC_SETINITIALCRAFTINGVALUES__TANGIBLEOBJECT_MANUFACTURESCHEMATIC_INT_,RPC_GETCREATIONCOUNT__MANUFACTURESCHEMATIC_};
...
int CraftingManager::getCreationCount(ManufactureSchematic* manufactureSchematic) {
	...
	if (unlikely(_implementation == NULL)) {
		...
		DistributedMethod method(this, RPC_GETCREATIONCOUNT__MANUFACTURESCHEMATIC_);
		method.addObjectParameter(manufactureSchematic);

		return method.executeWithSignedIntReturn();
	} else {
		return _implementation->getCreationCount(manufactureSchematic);
	}
}
...
DistributedObjectServant* CraftingManager::_getImplementationForRead() const {
	return _impl;
}
...
void CraftingManagerAdapter::invokeMethod(uint32 methid, DistributedMethod* inv) {
	...
	case RPC_SENDDRAFTSLOTSTO__CREATUREOBJECT_INT_:
		{
			...
			sendDraftSlotsTo(player, schematicID);
	...
	case RPC_SENDRESOURCEWEIGHTSTO__CREATUREOBJECT_INT_:
		{
			...
			sendResourceWeightsTo(player, schematicID);
	...
	case RPC_CALCULATEASSEMBLYSUCCESS__CREATUREOBJECT_DRAFTSCHEMATIC_FLOAT_:
		{
			CreatureObject* player = static_cast<CreatureObject*>(inv->getObjectParameter());
			DraftSchematic* draftSchematic = static_cast<DraftSchematic*>(inv->getObjectParameter());
			float effectiveness = inv->getFloatParameter();
			
			int _m_res = calculateAssemblySuccess(player, draftSchematic, effectiveness);
	...
	case RPC_CALCULATEEXPERIMENTATIONFAILURERATE__CREATUREOBJECT_MANUFACTURESCHEMATIC_INT_:
		{
			CreatureObject* player = static_cast<CreatureObject*>(inv->getObjectParameter());
			ManufactureSchematic* manufactureSchematic = static_cast<ManufactureSchematic*>(inv->getObjectParameter());
			int pointsUsed = inv->getSignedIntParameter();
			
			int _m_res = calculateExperimentationFailureRate(player, manufactureSchematic, pointsUsed);
			resp->insertSignedInt(_m_res);
		}
		break;
	case RPC_CALCULATEEXPERIMENTATIONSUCCESS__CREATUREOBJECT_DRAFTSCHEMATIC_FLOAT_:
		{
			...
			int _m_res = calculateExperimentationSuccess(player, draftSchematic, effectiveness);
			resp->insertSignedInt(_m_res);
		}
		break;
	case RPC_GENERATESERIAL__:
		{
			...
			String _m_res = generateSerial();
			resp->insertAscii(_m_res);
		}
		break;
	case RPC_SETINITIALCRAFTINGVALUES__TANGIBLEOBJECT_MANUFACTURESCHEMATIC_INT_:
		{
			TangibleObject* prototype = static_cast<TangibleObject*>(inv->getObjectParameter());
			ManufactureSchematic* manufactureSchematic = static_cast<ManufactureSchematic*>(inv->getObjectParameter());
			int assemblySuccess = inv->getSignedIntParameter();
			
			setInitialCraftingValues(prototype, manufactureSchematic, assemblySuccess);
			
		}
		break;
	case RPC_GETCREATIONCOUNT__MANUFACTURESCHEMATIC_:
		{
			ManufactureSchematic* manufactureSchematic = static_cast<ManufactureSchematic*>(inv->getObjectParameter());
			
			int _m_res = getCreationCount(manufactureSchematic);
			resp->insertSignedInt(_m_res);
		}
		break;
	default:
...
void CraftingManagerAdapter::stop() {
	(static_cast<CraftingManager*>(stub))->stop();
}
...
int CraftingManagerAdapter::getCreationCount(ManufactureSchematic* manufactureSchematic) {
	return (static_cast<CraftingManager*>(stub))->getCreationCount(manufactureSchematic);
}
...
DistributedObjectPOD* CraftingManagerHelper::instantiatePOD() {
	return new CraftingManagerPOD();
}
...
CraftingManagerPOD::~CraftingManagerPOD() {
}

CraftingManagerPOD::CraftingManagerPOD(void) {
	_className = "CraftingManager";
}


void CraftingManagerPOD::writeObject(ObjectOutputStream* stream) {
	int _currentOffset = stream->getOffset();
	stream->writeShort(0);
	int _varCount = CraftingManagerPOD::writeObjectMembers(stream);
	stream->writeShort(_currentOffset, _varCount);
}

int CraftingManagerPOD::writeObjectMembers(ObjectOutputStream* stream) {
...
bool CraftingManagerPOD::readObjectMember(ObjectInputStream* stream, const uint32& nameHashCode) {
...
void CraftingManagerPOD::readObject(ObjectInputStream* stream) {
	uint16 _varCount = stream->readShort();
	for (int i = 0; i < _varCount; ++i) {
		uint32 _nameHashCode;
		TypeInfo<uint32>::parseFromBinaryStream(&_nameHashCode, stream);
		...
		if(CraftingManagerPOD::readObjectMember(stream, _nameHashCode)) {
		}
...
void CraftingManagerPOD::writeObjectCompact(ObjectOutputStream* stream) {
