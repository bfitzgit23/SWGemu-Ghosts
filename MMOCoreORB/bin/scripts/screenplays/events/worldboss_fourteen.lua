...
--////////////////////////////////////////////////////////
worldboss_fourteenScreenplay = ScreenPlay:new {
	numberOfActs = 1,
  	planet = "yavin4",
...
registerScreenPlay("worldboss_fourteenScreenplay", true)
-----------------------------
--Start World Boss ScreenPlay
-----------------------------
function worldboss_fourteenScreenplay:start()
...
-----------------------
function worldboss_fourteenScreenplay:spawnMobiles(spawnList)
	for k,v in ipairs(spawnList) do 
		...
		creature:setScreenPlayState(k, "worldboss_fourteenScreenplay")
		createObserver(OBJECTDESTRUCTION, "worldboss_fourteenScreenplay", "bossDead", pBoss)
	end
end
---------------------------------------------------------------
--The Boss Has Died Respawn WorldBoss With A New Dynamic Spawn
---------------------------------------------------------------
function worldboss_fourteenScreenplay:bossDead(pBoss)
	print("World Boss One Has Died")
	local creature = CreatureObject(pBoss)
	local listNum = creature:getScreenPlayState("worldboss_fourteenScreenplay")
	local v = self.spawnPoints[listNum]
	createEvent(120 * 1000, "worldboss_fourteenScreenplay", "KillBoss", pBoss, "")
	local pBoss = spawnMobile(self.planet, "WORLDBOSS", -1, math.random(v.radius) + v.x, 0, math.random(v.radius) + v.y, math.random(360), 0)
	local creature = CreatureObject(pBoss)
	print("Respawning World Boss One")
	creature:setScreenPlayState(listNum, "worldboss_fourteenScreenplay")
	createObserver(OBJECTDESTRUCTION, "worldboss_fourteenScreenplay", "bossDead", pBoss)
	return 0
end
-----------------------------------------------------------------------------
...
-----------------------------------------------------------------------------
function worldboss_fourteenScreenplay:KillBoss(pBoss)
