...
--////////////////////////////////////////////////////////
worldboss_tenScreenplay = ScreenPlay:new {
	numberOfActs = 1,
  	planet = "",
...
registerScreenPlay("worldboss_tenScreenplay", true)
-----------------------------
--Start World Boss ScreenPlay
-----------------------------
function worldboss_tenScreenplay:start()
...
-----------------------
function worldboss_tenScreenplay:spawnMobiles(spawnList)
	for k,v in ipairs(spawnList) do 
		...
		creature:setScreenPlayState(k, "worldboss_tenScreenplay")
		createObserver(OBJECTDESTRUCTION, "worldboss_tenScreenplay", "bossDead", pBoss)
	end
end
---------------------------------------------------------------
--The Boss Has Died Respawn WorldBoss With A New Dynamic Spawn
---------------------------------------------------------------
function worldboss_tenScreenplay:bossDead(pBoss)
	print("World Boss One Has Died")
	local creature = CreatureObject(pBoss)
	local listNum = creature:getScreenPlayState("worldboss_tenScreenplay")
	local v = self.spawnPoints[listNum]
	createEvent(120 * 1000, "worldboss_tenScreenplay", "KillBoss", pBoss, "")
	local pBoss = spawnMobile(self.planet, "WORLDBOSS", -1, math.random(v.radius) + v.x, 0, math.random(v.radius) + v.y, math.random(360), 0)
	local creature = CreatureObject(pBoss)
	print("Respawning World Boss One")
	creature:setScreenPlayState(listNum, "worldboss_tenScreenplay")
	createObserver(OBJECTDESTRUCTION, "worldboss_tenScreenplay", "bossDead", pBoss)
	return 0
end
-----------------------------------------------------------------------------
...
-----------------------------------------------------------------------------
function worldboss_tenScreenplay:KillBoss(pBoss)
