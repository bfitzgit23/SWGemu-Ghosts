...
function Yavin4LightEnclaveScreenPlay:notifyEnteredEnclave(pArea, pLifeform)
	...
	if (SceneObject(pLifeform):isPlayerCreature()) then
		...
		-- Must be padawan, knight, master or master_phase2 (training phase)
		if status ~= "padawan" and status ~= "knight" and status ~= "master" and status ~= "master_phase2" then
			CreatureObject(pLifeform):sendSystemMessage("\\#FF4444 You have not yet awakened to the Force. The enclave denies you entry.")
			CreatureObject(pLifeform):teleport(self.forceField.x, self.forceField.z, self.forceField.y, 0)
		-- Only boot players who have EXPLICITLY chosen the dark side.
		-- Players with no alignment (nil/"") are seeking the trial and are allowed in.
		elseif alignment == "dark" then
			CreatureObject(pLifeform):sendSystemMessage("\\#FF4444 The light side recoils from the darkness within you. Seek the Dark Jedi Enclave.")
			CreatureObject(pLifeform):teleport(self.forceField.x, self.forceField.z, self.forceField.y, 0)
		else
			-- Determine rank label - Grand Master progression takes priority
			local rank
			local creature = CreatureObject(pLifeform)
			if creature:hasSkill("jedi_grand_master_novice") then
				rank = "Grand Master"
			else
				rank = (status == "master" and "Master" or status == "knight" and "Knight" or "Padawan")
			end
			if alignment == "" or alignment == nil then
				CreatureObject(pLifeform):sendSystemMessage("\\#88CCFF The Force stirs within you, " .. rank .. ". Seek the Challenge Terminal to begin your trial.")
			else
				CreatureObject(pLifeform):sendSystemMessage("\\#88CCFF The Force welcomes you, " .. rank .. ".")
			end
		end
	end
...
function Yavin4LightEnclaveScreenPlay:notifyEnteredTierOneRoom(pArea, pJedi)
	...
	local trialWave = readData("LightKnightTrial:" .. SceneObject(pJedi):getObjectID() .. ":wave")
	if trialWave ~= nil and trialWave == -1 then
		writeData("LightKnightTrial:" .. SceneObject(pJedi):getObjectID() .. ":wave", 1)
		LightEnclaveKnight:spawnWave(pJedi, 1)
		return 0
	end

	-- Skip rank check if trial is actively running
