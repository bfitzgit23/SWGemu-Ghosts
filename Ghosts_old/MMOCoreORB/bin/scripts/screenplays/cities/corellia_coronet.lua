...
-- ------------------------------------------------------------
-- Safety helpers (prevents nil-template CRC 0x0 spam and nil mood calls)
-- ------------------------------------------------------------
local function _trimStr(s)
...
function CorelliaCoronetScreenPlay:setMoodSafe(pNpc, mood)
	if pNpc ~= nil then
		self:setMoodSafe(pNpc, mood)
	end
end

function CorelliaCoronetScreenPlay:safeSpawnMobile(template, respawn, x, y, z, heading, cell, fallbacks)
	local function trySpawn(tmpl)
		tmpl = _trimStr(tmpl)
		if tmpl == nil then return nil end
		local ok, res = pcall(spawnMobile, self.planet, tmpl, respawn, x, y, z, heading, cell)
	...
	local res = trySpawn(template)
	if res ~= nil then return res end

	if type(fallbacks) == "table" then
		for _, ft in ipairs(fallbacks) do
			res = trySpawn(ft)
			if res ~= nil then return res end
		end
	end

	print(string.format("[CorelliaCoronetScreenPlay] WARNING - failed to spawn template '%s' on planet '%s'", tostring(template), tostring(self.planet)))
	return nil
end
...
function CorelliaCoronetScreenPlay:spawnMobiles()
	...
	self:setMoodSafe(pNpc, "conversation")
	...
	spawnMobile(self.planet, "chassis_dealer", 0, 0.1, 0.6, 72.9, -4, 1855672)
	  ...
	  -- Vendor Testing
	  self:safeSpawnMobile("risvendor", 1, -143.774, 28, -4723.66, 80, 0, {"ris_vendor","riss_vendor","rissvendor","risVendor"})
