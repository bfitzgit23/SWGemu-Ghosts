...
void AiAgentImplementation::loadTemplateData(CreatureTemplate* templateData) {
	...
	if (newlvl1 > 350) newlvl1 = 350;

	level = newlvl1; //getTemplateLevel();
...
//	}
	...
	float minDmg = newlvl1 * 3;//npcTemplate->getDamageMin(); //level * 6; //
	float maxDmg = newlvl1 * 5;//npcTemplate->getDamageMax(); //level * 10; //
	//speed for ai now in combat manager
	float speed = calculateAttackSpeed(level);//(5.0 - ((newlvl1 / 350) * 4));//calculateAttackSpeed(level);
...
//				}
	...
	float lvlmult = ((float)newlvl1 / (float)350);

	int lvlham = (35000 * lvlmult) - 750;//50 * (newlvl * .4) * 1.5;//newlvl * (newlvl * .15) + 250; // newlvl * (5 + (newlvl / 5));
